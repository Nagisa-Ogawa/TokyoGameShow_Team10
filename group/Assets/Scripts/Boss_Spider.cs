...
public class Boss_Spider : Enemy
{
    public enum BOSS_SPIDER_MODE {
        WAIT,
        MOVE_ATTACK,
    ...
    public BOSS_SPIDER_MODE m_mantisMode { get; private set; }
    ...
    void Start()
    {
        m_mantisMode = BOSS_SPIDER_MODE.WAIT;
        ...
        m_maxHP = m_HP;
        // 敵を事前に作成
        var enemies = m_enemyController.CreateEnemyBoss(4, m_createEnemyNum);
        ...
        switch (m_mantisMode)
        {
            case BOSS_SPIDER_MODE.WAIT:
                ...
                if (CheckTerritory())
                {
                    ...
                    if (FindTarget())
                    {
                        m_mantisMode = BOSS_SPIDER_MODE.MOVE_ATTACK;
                    }
                }
                else
                {
                    m_mantisMode = BOSS_SPIDER_MODE.MOVE_TERRITORY;
                }
                break;
            case BOSS_SPIDER_MODE.MOVE_ATTACK:
                if (m_target == null)
                {
                    ResetEnemy();
                    break;
                ...
                if (m_enemyAttack.m_HitObjList.Count > 0)
                {
                    ...
                    {
                        case 0:
                            m_mantisMode = BOSS_SPIDER_MODE.WAIT_RANGE_ATTACK_COOLDWON;
                            break;
                        case 1:
                            m_mantisMode = BOSS_SPIDER_MODE.WAIT_RANGE_ATTACK_COOLDWON;
                            break;
                        case 2:
                            m_mantisMode = BOSS_SPIDER_MODE.CREATE_ENEMY;
                            break;
                    }
                }
                else
                ...
                break;
            case BOSS_SPIDER_MODE.WAIT_RANGE_ATTACK_COOLDWON:
                // 攻撃可能なら攻撃へ
                if (CheckCanRangeAttack())
                {
                    m_mantisMode = BOSS_SPIDER_MODE.SETUP_RANGE_ATTACK;
                    if (m_choice == 0)
                    {
                ...
                break;
            case BOSS_SPIDER_MODE.SETUP_RANGE_ATTACK:
                break;
            case BOSS_SPIDER_MODE.RANGE_ATTACK:
                break;
            case BOSS_SPIDER_MODE.MOVE_TERRITORY:
                // 攻撃検知エリア内にミニオンがいるなら
                if (m_enemyAttack.m_HitObjList.Count > 0)
                {
                    // できたら自分から一番近いやつを選ぶ
                ...
                break;
            case BOSS_SPIDER_MODE.CREATE_ENEMY:
                ...
                break;
            case BOSS_SPIDER_MODE.AFTER_ATTACK:
                break;
            case BOSS_SPIDER_MODE.DEAD:
                break;
        }
    }
    ...
    public override void UpdateMove()
    {
        ...
        if (m_mantisMode == BOSS_SPIDER_MODE.MOVE_TERRITORY)
        {
    ...
    private IEnumerator ChargeEnemyRangeAttack(EnemyRangeAttack rangeAttack, SpriteRenderer renderer)
    {
        ...
        while (true)
        {
            ...
            if (ratio > 0.8f)
            {
                ...
                m_nowTime = 0.0f;
                // 攻撃コルーチンへ
                m_mantisMode = BOSS_SPIDER_MODE.RANGE_ATTACK;
    ...
    IEnumerator Wait()
    {
        m_mantisMode = BOSS_SPIDER_MODE.AFTER_ATTACK;
    ...
    public override void Damage(int damage)
    {
        ...
        else
        {
            ...
            // ターゲットを変更
            m_mantisMode = BOSS_SPIDER_MODE.DEAD;
